...
int main() {
    ...
    const char* output_file_name = "grayscale_mars_sequential.bmp";
    ...
    input_stream.read(full_header + 14 + sizeof(info_header_size), info_header_size - sizeof(info_header_size));
    if (!input_stream) {
    ...
    for (int row_idx = 0; row_idx < bmp_height; row_idx++) {
        int row_index = top_down ? row_idx : (bmp_height - 1 - row_idx);
        int row_start = row_index * row_size;
        for (int col_idx = 0; col_idx < bmp_width; col_idx++) {
            int pixel_idx = row_start + col_idx * 3;
            unsigned char b = static_cast<unsigned char>(pixel_data[pixel_idx]);
    ...
    cout << "Time: " << elapsed_time.count() << " microseconds - SEQUENTIAL" << endl;

    uint32_t new_file_size = full_header_size + image_size;
